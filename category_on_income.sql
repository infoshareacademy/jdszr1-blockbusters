...
c."name",
		...
		--korelacja kategorii filmu i sumy przychodów z filmu
		(select 
		...
		from (
				...
				where c.category_id = cfp.category_id) as subquery_5),
				
		--różnica średniego przychodu z filmów w których kategorii nie ma i w których kategoria jest
		(select 
